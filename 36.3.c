...
the_employees_t* read_from_file(size_t* size_array)
{
	the_employees_t* a;
	FILE *f = NULL;
	f = fopen("employees.txt", "a+b");
	if (f == NULL)
	{
		printf("Not opened\n");
		return NULL;
	...
	a = (the_employees_t*)malloc(size_f);
	fread(a, size_f, 1, f);
	fclose(f);
	return a;
}

the_employees_t* write_to_file()
{
	the_employees_t* a;
	FILE *f = NULL;
	f = fopen("employees.txt", "a+b");
...
the_employees_t* enter_date(the_employees_t* a, size_t* size_array) {
	if (a == NULL) {
		a = (the_employees_t*)malloc(sizeof(a)*1);
		size_array++;
	}
	for (int8_t i = 0; i <= size_array; i++)
	{
		printf("Enter surname\n");
		scanf_s("%s", &a[i].surname);
		printf("Enter age\n");
		scanf_s("%d", &a[i].age);
...
void* print_struct(the_employees_t* a, size_t* array_size)
{
...
void* edit_employees(the_employees_t* a, size_t* array_size)
{
	int position = -1;
	printf("Enter number of position for edit");
	scanf_s("%d", &position);
	printf("Enter surname\n");
	scanf_s("%s", &a[position].surname);
	printf("Enter color\n");
	scanf_s("%d", &a[position].age);
}

void* find_surname(the_employees_t* a, size_t* array_size)
{
	char surname_for_find[MAXSURNAME];
	scanf_s("%s", &surname_for_find);
	for (int index = 0; index < array_size; index++)
	{
